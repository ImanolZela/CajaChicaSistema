...
public class RegistroRendicionGastos extends JInternalFrame {
	...
	private ArrayList<RegistroRendicionGastosModel> registroRendicionGastosModelArrayList = new ArrayList<>();
	...
	public RegistroRendicionGastos() {
		...
		JComboBox<TipoCategoriaModel> cbmtipocategoria = new JComboBox<TipoCategoriaModel>();
		cbmtipocategoria.setBounds(184, 61, 183, 25);
		getContentPane().add(cbmtipocategoria);

		JComboBox<TipoComprobanteModel> cbmtipocomprobante = new JComboBox<TipoComprobanteModel>();
		...
		addInternalFrameListener(new InternalFrameAdapter() {
			...
			public void internalFrameOpened(InternalFrameEvent e) {
				...
				RegistroCajaChicaModel miCajaChica = new RegistroCajaChicaRepository().listarProyecto(VariablesGlobales.USUARIO_ID);
				...
				cbmtipocategoria.addItem(new TipoCategoriaModel());
				for (TipoCategoriaModel tipoCategoria : tipoCategoriaArrayList) {
					cbmtipocategoria.addItem(tipoCategoria);
				}

				tipoComprobanteArrayList = new TipoComprobanteRepository().obtenerTipoComprobante();

				cbmtipocomprobante.addItem(new TipoComprobanteModel());
				for (TipoComprobanteModel tipoComprobante : tipoComprobanteArrayList) {
		...
		btnAgregar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				RegistroRendicionGastosModel registroRendicion;

				TipoCategoriaModel tipoCategoriaSeleccionado = (TipoCategoriaModel) cbmtipocategoria.getSelectedItem();
				TipoComprobanteModel tipoComprobanteSeleccionado = (TipoComprobanteModel) cbmtipocomprobante
						.getSelectedItem();
				...
				if (tipoCategoriaSeleccionado != null && tipoComprobanteSeleccionado != null) {
					...
					try {
						...
						registroRendicion = new RegistroRendicionGastosModel(caja_id, categoria_id, tipo_comprobante_id,
		...
		btnGuardar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					for (RegistroRendicionGastosModel registro : registroRendicionGastosModelArrayList) {
