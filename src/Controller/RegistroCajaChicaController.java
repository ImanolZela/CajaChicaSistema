...
public class RegistroCajaChicaController {
	...
	private ArrayList<RegistroCajaChicaModel> cajasChicasArrayList = new ArrayList<RegistroCajaChicaModel>();

	public RegistroCajaChicaController(Connection cnn) {
		this.cnn = cnn;
	}

	public void insertar(RegistroCajaChicaModel registroCajaChica) {
	...
	public RegistroCajaChicaModel listarCajaPorUsuario(int usuario_id) {
		PreparedStatement sentencia = null;
		ResultSet rs = null;
		...
		try {
			...
			while (rs.next()) {
				...
				registroCajaChicaProyecto = new RegistroCajaChicaModel(caja_id_t, usuario_id_t, nombre_proyecto_t,
						monto_asignado_t);
